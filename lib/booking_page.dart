...
class _BookingPageState extends State<BookingPage> {
  ...
  final List<int> _durationOptions = [30, 60, 90, 120, 180]; // Options in minutes
  ...
  Future<void> _loadRooms() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _showSnackBar('Error loading rooms. Please try again.',
            color: Colors.red);
      }
    }
  }
  ...
  void _filterRoomsByType() {
    List<Room> filteredRooms = _availableRooms
        .where((room) => room.type == _selectedRoomType)
        .toList();
      ...
      if (_selectedRoom != null) {
        for (var feature in RoomFeature.values) {
          _selectedFeatures[feature] =
              _selectedRoom!.features.contains(feature);
        }
      } else {
        _resetFeatures();
      }
  ...
  Future<bool> _isRoomAvailableAtSelectedTime() async {
    ...
    // Modified to use async Firebase check
    return await _bookingService.isRoomAvailable(
        _selectedRoom!.id, selectedDateTime!);
  }
  ...
  void _selectDateTime(BuildContext context) async {
    final DateTime? pickedDate = await showDatePicker(
      ...
      builder: (context, child) =>
          _buildDateTimePickerTheme(context, child, 'Date picker for booking'),
    );
    ...
    final TimeOfDay? pickedTime = await showTimePicker(
      ...
      builder: (context, child) =>
          _buildDateTimePickerTheme(context, child, 'Time picker for booking'),
    );
  ...
  Widget _buildDateTimePickerTheme(
      BuildContext context, Widget? child, String label) {
    return Theme(
      ...
      child: Semantics(
        label: label,
        hint:
            'Select a ${label.contains('Date') ? 'date' : 'time'} for your booking',
        child: child!,
      ),
    );
  }
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          actions: [
            _buildDialogButton(
                'Cancel', () => Navigator.of(dialogContext).pop(), Colors.grey),
  ...
  Future<void> _confirmBooking() async {
    ...
    try {
      ...
      final newBooking = Booking(
        ...
        duration: _duration, // Use selected duration
      );

      // Add booking to Firebase
      bool success = await _bookingService.addBooking(newBooking);
      ...
      if (success) {
        ...
        _showSnackBar('Error: Failed to save booking', color: Colors.red);
      }
    } catch (e) {
      if (!mounted) return;
  ...
  void _showSnackBar(String message,
      {Color color = Colors.black, Duration? duration}) {
  ...
  Future<void> _showExistingBookings() async {
    if (_selectedRoom == null) {
    ...
    try {
      ...
      final roomBookings = await _bookingService.getRoomBookings(_selectedRoom!.id);
              ...
              child: futureBookings.isEmpty
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final formattedTime = DateFormat('EEEE, MMM d')
                            .add_jm()
          ...
          content: SingleChildScrollView(
            child: ListBody(
              children: <Widget>[
                ...
                _buildHelpItem(
                    '1. Select a room type (Quiet Room, Conference Room, or Study Room)'),
                _buildHelpItem(
                    '2. Choose a specific room from the available options'),
                _buildHelpItem('3. Select a date and time for your booking'),
                _buildHelpItem('4. Choose a duration for your booking'),
                _buildHelpItem('5. Review and confirm your booking details'),
                const SizedBox(height: 10),
                const Text(
                ...
                _buildHelpItem(
                    '• Use your device\'s screen reader to navigate through the booking process'),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (_selectedRoom != null &&
                            _selectedRoom!.features.isNotEmpty)
                          _buildRoomFeatures(),
      ...
      child: Card(
        ...
        child: Column(
          children: RoomType.values
              .map((type) => _buildRoomTypeOption(type))
              .toList(),
        ),
      ),
    );
  ...
  Widget _buildRoomTypeOption(RoomType type) {
    ...
    final availableCount =
        _availableRooms.where((room) => room.type == type).length;
      ...
      onChanged: availableCount > 0
          ? (RoomType? value) {
              if (value != null) {
                ...
                _showSnackBar('Selected ${value.displayName}',
  ...
  Widget _buildRoomsSelector() {
    // Filter rooms by selected type
    final roomsOfSelectedType = _availableRooms
        .where((room) => room.type == _selectedRoomType)
        .toList();
    ...
    return Card(
      ...
      child: Column(
        children:
            roomsOfSelectedType.map((room) => _buildRoomOption(room)).toList(),
      ),
    );
  }
          ...
          Padding(
            ...
            child: Align(
              ...
              child: TextButton.icon(
                ...
                label: Text('View Existing Bookings',
            ...
            child: Column(
              ...
              children: [
                ...
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: _selectedRoom!.features
                      .map((feature) => Chip(
      ...
      children: [
        ...
        Semantics(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: _isRoomAvailable ? primaryColor.withAlpha(26) : Colors.red.withAlpha(26),
              borderRadius: BorderRadius.circular(10),
              border: Border.all(
                color: _isRoomAvailable ? primaryColor.withAlpha(128) : Colors.red.withAlpha(128)
              ),
            ),
            child: Row(
              children: [
                Icon(
                  _isRoomAvailable ? Icons.event_available : Icons.event_busy,
                  color: _isRoomAvailable ? primaryColor : Colors.red,
                ),
                const SizedBox(width: 10),
                Expanded(
                  child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              children: _durationOptions.map((duration) {
                final bool isSelected = _duration == duration;
                return ChoiceChip(
                  label: Text('$duration min'),
                  selected: isSelected,
                  selectedColor: primaryColor.withAlpha(200),
            ...
            SizedBox(height: 10),
            Text(
              'Current selection: $_duration minutes',
              style: TextStyle(
  ...
  Widget _buildConfirmButton() {
    return Focus(
      ...
      child: Semantics(
        ...
        enabled: _canConfirmBooking() && !_isLoading,
        child: ElevatedButton.icon(
          ...
          onPressed: (!_canConfirmBooking() || _isLoading) 
              ? null 
